...
def get_train_dataloader(file_path,img_dir,batch_size):
    ...
    dataloader = DataLoader(transformed_labdataset_tensor, batch_size=batch_size,
...
def get_val_dataloader(file_path,img_dir,batch_size):
    transformed_labdataset_tensor = LabFaceDataset(annotations_file=file_path,
                                            img_dir=img_dir,)                                        
    #batch_size to the data can be modified here by adjusting the argument batch_size(Here 4)
    dataloader = DataLoader(transformed_labdataset_tensor, batch_size=batch_size,
...
def run_training():
    ...
    optimizer = {'Adam':torch.optim.Adam(model.parameters(), lr=1e-2, weight_decay=0.0),'SDG':torch.optim.SGD(model.parameters(), lr=0.01, momentum=0.9)}
    ...
    for key,value in optimizer.items():
        for e,b in zip(epochs_l,batch_size_l):
            print("With Batch size",b)
            train_loader = get_train_dataloader(annotate_file_path_train, img_dir,b)
            val_loader = get_val_dataloader(annotate_file_path_test, img_dir,b)
            # Train model
            print("Using optimizer",key)
            print("With number of epochs to be",e)
            for epoch in range(e):
            # Training and validationg for each epoch
                train(train_loader, model, criterion, value, epoch)
                with torch.no_grad():
